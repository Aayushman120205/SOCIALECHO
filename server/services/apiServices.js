...
const getCategoriesFromTextRazor = async (content, timeout) => {
  ...
  try {
    ...
    if (axios.isCancel(error)) {
      ...
      const { status, statusText } = error.response || { status: "Unknown", statusText: "Error" };
      console.error(`TextRazor API Error ${status}: ${statusText} - Falling back to no categories`);
      return categories;
    }
  } finally {
...
const getCategoriesFromClassifierAPI = async (content, timeout) => {
  ...
  try {
    ...
    if (axios.isCancel(error)) {
      ...
      const { status, statusText } = error.response || { status: "Unknown", statusText: "Error" };
      console.error(`Classifier API Error ${status}: ${statusText} - Falling back to no categories`);
      return categories;
    }
  } finally {
    clearTimeout(timeoutId);
...
const getCategoriesFromInterfaceAPI = async (content, timeout) => {
  ...
  try {
    ...
    if (axios.isCancel(error)) {
      ...
      const { status, statusText } = error.response || { status: "Unknown", statusText: "Error" };
      console.error(`Interface API Error ${status}: ${statusText} - Falling back to no categories`);
      return categories;
    }
  } finally {
    clearTimeout(timeoutId);
